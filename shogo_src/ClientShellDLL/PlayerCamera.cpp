...
void CPlayerCamera::MoveCameraToPosition(DVector pos, DFLOAT deltaTime)
{
	if (!m_pClientDE || !m_hTarget) return;
	...
	if (m_eCameraMode == GOINGFIRSTPERSON)
	{
		...
		DFLOAT percentage = (nCurrentTime - m_GoingFirstPersonStart) / (m_GoingFirstPersonTransition);
...
void CPlayerCamera::StartCircle(DFLOAT HeightOffset, DFLOAT Radius, DFLOAT PointAtOffset, DFLOAT Time)
{
	if (!m_pClientDE || !m_hTarget) return;
...
void CPlayerCamera::CircleAroundTarget()
{
	...
	DFLOAT diff = (m_pClientDE->GetTime() - m_CircleStartTime) / m_CircleTime;
