...
DBOOL CDetailSettingsMenu::Init (CClientDE* pClientDE, CRiotMenu* pRiotMenu, CBaseMenu* pParent, int nScreenWidth, int nScreenHeight)
{
...
void CDetailSettingsMenu::Draw (HSURFACE hScreen, int nScreenWidth, int nScreenHeight, int nTextOffset)
{
	...
	for (int i = m_nTopItem; i < RS_SUBDET_LAST + 2; i++)
	{
		...
		{
			m_pClientDE->DrawSurfaceToSurfaceTransparent (hScreen, m_nSelection == i ? m_DetailSetting[i].hMenuItemSelected : m_DetailSetting[i].hMenuItem, DNULL, m_nMenuX + m_nSecondColumn, y, DNULL);
		}
...
DBOOL CDetailSettingsMenu::LoadSurfaces()
{
	...
	for (i = RS_SUBDET_FIRST; i <= RS_SUBDET_LAST + 1; i++)
	{
...
void CDetailSettingsMenu::UnloadSurfaces()
{
	...
	for (int i = RS_SUBDET_FIRST; i <= RS_SUBDET_LAST + 1; i++)
	{
		...
		m_DetailSetting[i].hMenuItem = DNULL;
		m_DetailSetting[i].hMenuItemSelected = DNULL;
	}
